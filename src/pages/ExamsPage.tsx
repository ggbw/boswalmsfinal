...
export default function ExamsPage() {
  ...
  const handleEnterMarks = (exam: typeof exams[0]) => {
    ...
    students.forEach(s => {
      const existing = db.marks.find(m => m.studentId === s.studentId && m.moduleId === exam.moduleId && m.classId === exam.classId);
    ...
    showModal(`Enter Marks — ${exam.name}`, (
      <div>
        ...
        <div className="table-wrap" style={{ maxHeight: 400, overflowY: 'auto' }}>
          <table>
            <thead><tr><th>Student</th><th>ID</th><th>Mark (0–100)</th></tr></thead>
            <tbody>
              {students.map(s => (
                <tr key={s.id}>
        ...
        <button className="btn btn-primary" style={{ marginTop: 14 }} onClick={async () => {
          ...
          for (const s of students) {
            ...
            const existing = db.marks.find(m => m.studentId === s.studentId && m.moduleId === exam.moduleId && m.classId === exam.classId);
            if (existing) {
              const { error } = await supabase.from('marks').update({ final_exam: mark })
                .eq('student_id', s.studentId).eq('module_id', exam.moduleId).eq('class_id', exam.classId);
              if (error) hasError = true;
            } else {
              const { error } = await supabase.from('marks').insert({
                student_id: s.studentId, module_id: exam.moduleId, class_id: exam.classId,
                test1: 0, test2: 0, pract_test: 0, ind_ass: 0, grp_ass: 0,
                final_exam: mark, practical: 0,
