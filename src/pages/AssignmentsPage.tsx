...
export default function AssignmentsPage() {
  ...
  if (role === "student" && currentStudent) {
    ...
    const overrideModuleIds = db.studentModules
      .filter((sm) => sm.studentId === currentStudent.id)
      .map((sm) => sm.moduleId);
    ...
    showModal(
      ...
      <div>
        <div className="form-group">
          <label>Title *</label>
          <input className="form-input" onChange={(e) => (title = e.target.value)} />
        </div>
        <div className="form-group">
          <label>Description</label>
          <textarea className="form-input" rows={3} onChange={(e) => (description = e.target.value)} />
        </div>
        ...
        <div className="form-row cols2">
          <div className="form-group">
            ...
            <input className="form-input" type="date" onChange={(e) => (dueDate = e.target.value)} />
          </div>
          <div className="form-group">
            ...
            <input
              ...
              type="number"
              defaultValue={100}
              onChange={(e) => (marks = Number(e.target.value))}
            />
          </div>
        </div>
        <div className="form-group">
          ...
          <select
            ...
            onChange={(e) => (submissionType = e.target.value)}
          >
        ...
        <div className="form-group">
          ...
          <div style={{ fontSize: 11, color: "var(--text2)", marginTop: 4 }}>
            Attach a reference document, rubric, or instructions file (max 10MB)
          </div>
        </div>
        <button
          ...
          onClick={async () => {
            if (!title || !moduleId) {
              toast("Title and module are required", "error");
              return;
            }
            if (attachmentFile && attachmentFile.size > 10 * 1024 * 1024) {
              toast("Attachment must be under 10MB", "error");
              return;
            }
  ...
  const handleGradeSubmission = (submission: any, assignment: any) => {
    ...
    showModal(
      `Grade: ${db.students.find((s) => s.id === submission.studentId)?.name || "Student"}`,
      <div>
        <div className="form-row cols2">
          <div className="form-group">
            ...
            <input
              ...
              defaultValue={grade}
              onChange={(e) => (grade = Number(e.target.value))}
            />
          </div>
          <div className="form-group">
            <label>Status</label>
        ...
        <div className="form-group">
          ...
          <textarea
            ...
            onChange={(e) => (feedback = e.target.value)}
          />
        </div>
        <button
          className="btn btn-primary"
      ...
      <div className="page-header">
        <div>
          <div className="page-title">
            <i className="fa-solid fa-list-check" style={{ color: "var(--accent)", marginRight: 8 }} />
            Assignments
          </div>
          <div className="page-sub">{assignments.length} assignment(s)</div>
        </div>
        {(role === "lecturer" || role === "admin") && (
          <button className="btn btn-primary btn-sm" onClick={handleCreateAssignment}>
          ...
          <table>
            <thead>
              <tr>
                ...
                {(role === "lecturer" || role === "admin") && <th>Actions</th>}
              </tr>
            </thead>
            <tbody>
              {assignments.map((a) => {
                ...
                const mySubmission = currentStudent
                  ? db.submissions.find((sub) => sub.assignmentId === a.id && sub.studentId === currentStudent.id)
                  : null;
                ...
                return (
                  <tr key={a.id} onClick={() => handleViewAssignment(a.id)} style={{ cursor: "pointer" }}>
                    ...
                    <td>
                      <span className={`badge ${a.submissionType === "hardcopy" ? "badge-pending" : "badge-active"}`}>
                        {(a.submissionType || "softcopy").charAt(0).toUpperCase() +
                          (a.submissionType || "softcopy").slice(1)}
                      </span>
                    </td>
                    <td>
                      {mySubmission ? (
                        ...
                        <span
                          className={`badge ${a.status === "graded" ? "badge-credit" : a.status === "active" ? "badge-pass" : "badge-inactive"}`}
                        >
                          {a.status}
                        </span>
                      )}
            ...
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <div className="info-row">
                  ...
                  <span className="info-val">{a.dueDate || "—"}</span>
                </div>
                <div className="info-row">
                  <span className="info-label">Total Marks</span>
                  <span className="info-val">{a.marks}</span>
                </div>
                <div className="info-row">
                ...
                <div className="info-row">
                  ...
                  <span className="info-val">{a.status}</span>
                </div>
                {a.uploadedBy && (
                  <div className="info-row">
                    <span className="info-label">Created By</span>
                    <span className="info-val">{a.uploadedBy}</span>
                  </div>
                )}
                {a.uploadedDate && (
                  <div className="info-row">
                    <span className="info-label">Created Date</span>
                    <span className="info-val">{a.uploadedDate}</span>
                  </div>
                )}
              </div>
              {a.description && (
                ...
                <div style={{ marginTop: 16, padding: 12, background: "var(--bg2)", borderRadius: 8 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div className="info-row">
                      ...
                      <span className="info-val">{mySubmission.fileName}</span>
                    </div>
                    <div className="info-row">
                      <span className="info-label">Size</span>
                      <span className="info-val">{mySubmission.fileSize}</span>
                    </div>
                    <div className="info-row">
                      <span className="info-label">Submitted</span>
                      <span className="info-val">
                        {mySubmission.submittedDate} {mySubmission.submittedTime}
                      </span>
                    </div>
                    <div className="info-row">
                      <span className="info-label">Status</span>
                      <span className="info-val">
                        <span className="badge badge-credit">{mySubmission.status}</span>
                      </span>
                    </div>
                    ...
                    {mySubmission.feedback && (
                      <div className="info-row" style={{ gridColumn: "1/-1" }}>
                        ...
                        <span className="info-val">{mySubmission.feedback}</span>
                      </div>
                    )}
                  </div>
                </div>
                          ...
                          {allSubmissions.map((sub) => {
                            ...
                            return (
                              <tr key={sub.id}>
                                ...
                                <td>
                                  <span
                                    className={`badge ${sub.status === "graded" ? "badge-credit" : "badge-pending"}`}
                                  >
                                    {sub.status}
                                  </span>
                                </td>
                                ...
                                <td onClick={(e) => e.stopPropagation()}>
                                  <button
                                    ...
                                    onClick={() => handleGradeSubmission(sub, a)}
                                  >
