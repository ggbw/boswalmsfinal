...
export default function AttendancePage() {
  ...
  const [attClass, setAttClass] = useState(db.classes[0]?.id || '');
  ...
  const setAtt = (id: string, status: string) => setAttState(prev => ({ ...prev, [id]: status }));
  const saveAttendance = () => {
    const records = students.map(s => ({ studentId: s.studentId, classId: attClass, date: attDate, status: attState[s.id] || 'present' }));
    setDb(prev => ({ ...prev, attendance: [...prev.attendance, ...records] }));
  ...
  return (<>
    ...
    <div className="card" style={{marginBottom:14}}>
      <div className="form-row cols3">
        <div className="form-group"><label>Class</label><select className="form-select" value={attClass} onChange={e=>{setAttClass(e.target.value);setAttState({});}}>{db.classes.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        <div className="form-group"><label>Date</label><input className="form-input" type="date" value={attDate} onChange={e=>setAttDate(e.target.value)}/></div>
        <div className="form-group"><label>Module</label><select className="form-select">{db.modules.filter(m=>m.classes.includes(attClass)).map(m=><option key={m.id}>{m.name}</option>)}</select></div>
      </div>
      ...
      <div className="att-grid">{students.map(s=>(
        <div key={s.id} className="att-card"><div className="att-name">{s.name.split(' ').slice(0,2).join(' ')}</div>
