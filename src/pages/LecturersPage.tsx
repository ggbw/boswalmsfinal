...
interface FacultyRow {
  ...
  role: string; dept: string; code: string;
}
...
export default function LecturersPage() {
  ...
  const load = useCallback(async () => {
    ...
    const mapped = (profiles || [])
      ...
      .map((p: any) => ({
        ...
        role: roleMap[p.user_id] || 'lecturer', dept: p.dept || '', code: p.code || '',
      }));
    setFaculty(mapped);
    setLoading(false);
  }, []);
  ...
  const handleEdit = (f: FacultyRow) => {
    let name = f.name, email = f.email, dept = f.dept, code = f.code, role = f.role;
    showModal('Edit Lecturer — ' + f.name, (
      <div>
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 12, color: '#002060', borderBottom: '2px solid #C9A227', paddingBottom: 4, marginBottom: 12 }}>Account Details</div>
          ...
          <div className="form-row cols2">
            ...
            <div className="form-group"><label>Staff Code</label>
              <input className="form-input" defaultValue={code} placeholder="e.g. LEC001" onChange={e => code = e.target.value} />
            </div>
          </div>
        </div>
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 12, color: '#002060', borderBottom: '2px solid #C9A227', paddingBottom: 4, marginBottom: 12 }}>Department</div>
        ...
        <button className="btn btn-primary" style={{ width: '100%' }} onClick={async () => {
          ...
          const { error: profErr } = await supabase.from('profiles').update({ name, email, dept, code }).eq('user_id', f.user_id);
          if (profErr) { toast(profErr.message, 'error'); return; }
          if (role !== f.role) {
            const { error: roleErr } = await supabase.from('user_roles').update({ role: role as any }).eq('user_id', f.user_id);
            if (roleErr) { toast(roleErr.message, 'error'); return; }
          }
          toast('Lecturer updated!', 'success');
          closeModal();
          // Update local state immediately
          setFaculty(prev => prev.map(x => x.user_id === f.user_id ? { ...x, name, email, dept, code, role } : x));
          ...
          <div className="card" style={{ padding: 0 }}>
            {loading ? (
              ...
              <div className="table-wrap">
                <table>
                  <thead><tr><th>Name</th><th>Role</th><th>Department</th><th>Email</th><th>Status</th><th>Action</th></tr></thead>
                  <tbody>
                    {filtered.map(f => (
                      <tr key={f.user_id} style={{ background: selected?.user_id === f.user_id ? 'var(--bg2)' : undefined }}>
                        <td className="td-name">
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <div className="avatar" style={{ background: '#c8860a', width: 28, height: 28, fontSize: 11 }}>{f.name[0]}</div>
                            {f.name}
                          </div>
                        </td>
                        ...
                        <td>
                          <div style={{ display: 'flex', gap: 4 }}>
                            <button className="btn btn-outline btn-sm" onClick={() => setSelected(f)}>View</button>
                            {isAdmin && <button className="btn btn-outline btn-sm" onClick={() => handleEdit(f)}>Edit</button>}
                          </div>
                        </td>
                      </tr>
                    ))}
        ...
        <div style={{ flex: 1, background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'auto', padding: 24 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
              <div className="avatar" style={{ background: '#c8860a', width: 52, height: 52, fontSize: 20 }}>{selected.name[0]}</div>
              <div>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {isAdmin && <button className="btn btn-outline btn-sm" onClick={() => handleEdit(selected)}><i className="fa-solid fa-pen" /> Edit</button>}
              <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: 'var(--text2)' }} onClick={() => setSelected(null)}>×</button>
            </div>
          </div>
  ...
  return (
    <PanelSection title="Assigned Classes">
      {assignedClasses.map((c: any) => {
        ...
        return (
          <div key={c.id} style={{ gridColumn: '1/-1', background: 'var(--bg2)', borderRadius: 6, padding: '8px 12px', fontSize: 13 }}>
            <div style={{ fontWeight: 600 }}>{c.name}</div>
            <div style={{ color: 'var(--text2)', fontSize: 11, marginTop: 2 }}>{prog?.name || '—'} · Year {c.year} · Sem {c.semester}</div>
          </div>
        );
      })}
    </PanelSection>
